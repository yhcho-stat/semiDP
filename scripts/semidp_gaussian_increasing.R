# Gaussian Mechanism on Contingency table with the cell probabilities increase linearly
#rm(list = ls())
...
all_average_losses_increasing <- data.frame()

# Loop over k from 2 to
for (k in c(2:10)) {
  # Run the simulations and get the average losses
  average_losses_increasing <- run_multiple_simulations(num_simulations, mu, k, Sens, SensSemi, SemiAdj, n, seeds)
  # Store the results in the dataframe
  all_average_losses_increasing <- rbind(all_average_losses_increasing, 
                                         data.frame(k = k, LossSemiDP = average_losses_increasing[1], 
                                                    LossNaive = average_losses_increasing[2]))
}

# Remove row names
rownames(all_average_losses_increasing) <- NULL

# Sort the results by k
all_average_losses_increasing <- all_average_losses_increasing[order(all_average_losses_increasing$k), ]
